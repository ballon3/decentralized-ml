...
@pytest.fixture(scope='session')
def runner(config_manager, ipfs_client, blockchain_gateway):
    runner = DMLRunner(config_manager)
    runner.configure(ipfs_client=ipfs_client, blockchain_gateway=blockchain_gateway)
    return runner

def test_dmlrunner_communicate_job(runner, train_dmlresult_obj, ipfs_client):
    ...
    assert result.results["receipt"] is runner._blockchain_gateway.state
